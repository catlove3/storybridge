...
使用现有真实模型配置启动同域网站和临时 HTTPS 隧道。终端显示地址后，打开网站的“手机扫码体验”可放大、下载二维码；访客无需注册或填写 API。首次启动会加密现有模型密钥。请保持电脑和终端运行，按 Ctrl+C 停止。

页面提供“输入故事 → 选择改编方案 → 查看结果”三步流程，内置可编辑的教学短剧目录、匿名访客隔离、持久每日额度、失败阶段重试和刷新恢复。公开模式每访客每日默认 300,000 tokens，全站 100,000,000 tokens，北京时间零点恢复。临时地址可能变化，现场网络与微信真机仍需验收。

启动分享、配置固定地址、添加短剧、密钥备份、调整额度和故障处理请看 [手机扫码体验交付说明](docs/SHARING.md)。开发模拟分享可显式运行 `./speed_run.sh --share --mock`，页面会明确标识。
...
- 本地开发未配置 `STORYBRIDGE_API_KEYS` 时是单用户模式；`--share` 使用独立匿名会话、Cookie 与 CSRF，保留后端 API key 调用兼容性。
- SFT 全文采集默认关闭，必须同时开启部署开关并取得项目级来源、许可和同意。
- 常规 LLM 账本不保存 prompt 或 completion，只保存 BLAKE2b 指纹、模型、步骤、耗时、token 和成本估算。
- 本地模式默认每项目额度为 1,000,000 tokens；公开模式改用跨项目的每日访客和全站额度，可在 `backend/config/models.yaml` 调整。
