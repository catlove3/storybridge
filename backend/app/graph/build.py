...
class StoryGraph:
    ...
    def _build(self) -> None:
        ...
        for dep in self.state.dependencies:
            if dep.source_id not in self.graph or dep.target_id not in self.graph:
                continue
            for src, dst in _impact_edges(dep.relation, dep.source_id, dep.target_id):
                ...
                if existing is None or float(existing.get("confidence", 0)) < dep.confidence:
                    self.graph.add_edge(src, dst, **attr)

    def has_node(self, node_id: str) -> bool:
        return node_id in self.graph

    def display_subgraph(self, focus_ids: list[str] | None = None, depth: int = 2) -> nx.DiGraph:
        if not focus_ids:
