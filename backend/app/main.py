...
async def lifespan(app: FastAPI):
    ...
    logger.info(
        "StoryBridge paths: database=%s legacy_projects=%s legacy_jobs=%s sft_logs=%s run_logs=%s",
...
async def readyz(response: Response) -> dict:
    config = get_config()
    checks = {
        "database_storage": config.storage.database_file.parent.is_dir()
        and os.access(config.storage.database_file.parent, os.R_OK | os.W_OK),
