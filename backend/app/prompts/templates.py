...
def verify_consistency_user(
    ...
    return (
        ...
        "检查项：\n"
        "1. stale_reference：场景文本中实际出现已被替换机制的旧表述（原词或同义说法）。\n"
        "2. fact_conflict：人物事实/世界观自相矛盾。\n"
        "3. motivation_break：人物行为失去动机支撑。\n"
        "4. commitment_violation：叙事承诺被破坏。\n"
        "5. unresolved_payoff：伏笔失去回收。\n\n"
        "**关键规则**：evidence 必须逐字摘自上面的场景文本，禁止根据改编说明或你的推测构造引用。"
        "如果某场景文本中并没有出现旧机制表述，绝不要对该场景报告 stale_reference。\n\n"
        f"输出 JSON schema：\n{VERIFY_SCHEMA}\n\n"
