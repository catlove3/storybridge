...
test('mock backend full flow survives a page reload', async ({ page }) => {
  ...
  await expect(page.getByRole('button', { name: /E2E 恢复测试/ })).toHaveAttribute('aria-current', 'page')
  await page.getByRole('button', { name: /新建项目/ }).click()
  await expect(page.getByLabel('中文剧本')).toHaveValue('')
  await expect(page.getByText('等待输入剧本')).toBeVisible()
  await expect(page).not.toHaveURL(/\?project=/)
  expect(await page.evaluate(() => localStorage.getItem('storybridge.activeProjectId'))).toBeNull()

  await page.reload()
  await expect(page.getByText('等待输入剧本')).toBeVisible()
  await page.getByRole('button', { name: /^项目/ }).click()
  await page.getByRole('button', { name: /E2E 恢复测试/ }).click()
  await expect(page.getByText('2 个文化点 · 7 个场景已更新')).toBeVisible({ timeout: 30_000 })
})
